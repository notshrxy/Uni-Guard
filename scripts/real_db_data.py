...
class NavItem(QPushButton):
    ...
    def paintEvent(self, event):
        ...
        if getattr(self, '_show_alert', False) and self._name != "Configuration":
            ...
            p.drawText(QRectF(bx - badge_r, by - badge_r, badge_r * 2, badge_r * 2), Qt.AlignmentFlag.AlignCenter, "!")
            p.restore()
...
class DatabaseExplorerWidget(QWidget):
    ...
    def _setup_ui(self):
        ...
        # --- Column 2: Right Control Area (Month label & Circular Refresh Button) ---
        ...
        right_layout.addWidget(self.month_lbl)
    ...
    def _load_data(self, table_name):
        ...
        if table_name == "students":
            ...
            target_monday = monday_this_week - datetime.timedelta(weeks=offset)
    ...
    def _on_cell_changed(self, item):
        ...
        try:
            if self.active_table == "students":
                ...
                if 1 <= col <= 5:
                    day_cols = {1: "MON", 2: "TUE", 3: "WED", 4: "THU", 5: "FRI"}
                    db_col_name = day_cols.get(col)
                    upper_val = new_val.upper()
                    if upper_val not in ["P", "A"]:
                        upper_val = "A"
                    
                    selected_week = self.week_combo.currentText()
                    cursor.execute("SELECT id FROM weekly_attendance WHERE register_number = ? AND week_name = ?", (original_db_id, selected_week))
                    w_row = cursor.fetchone()
                    if w_row:
                        cursor.execute(f"UPDATE weekly_attendance SET {db_col_name} = ? WHERE id = ?", (upper_val, w_row[0]))
                    else:
                        days_vals = ["A"] * 7
                        day_idx_map = {1: 0, 2: 1, 3: 2, 4: 3, 5: 4}
                        days_vals[day_idx_map[col]] = upper_val
                        cursor.execute("""
                            INSERT INTO weekly_attendance (register_number, week_name, MON, TUE, WED, THU, FRI, SAT, SUN, total_present, attendance_percentage)
                            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, 0, 0.0)
                        """, (original_db_id, selected_week, *days_vals))
                    
                    # Recalculate total and percentage
                    cursor.execute("SELECT MON, TUE, WED, THU, FRI, SAT, SUN FROM weekly_attendance WHERE register_number = ? AND week_name = ?", (original_db_id, selected_week))
                    day_row = cursor.fetchone()
                    if day_row:
                        present_count = sum(1 for d in day_row if str(d).upper() == "P")
                        att_pct = round(present_count / 7 * 100, 2)
                        cursor.execute("UPDATE weekly_attendance SET total_present = ?, attendance_percentage = ? WHERE register_number = ? AND week_name = ?", (present_count, att_pct, original_db_id, selected_week))
            else: # attendance
                pass
            conn.commit()
        except Exception as e:
...
class UniGuardDashboard(QMainWindow):
    def __init__(self, model_path=None):
        ...
        # Initialize seed statistics
        self.total_scans = 14
        self.total_permitted = 78
